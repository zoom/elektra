...
func TestNewConfigError(t *testing.T) {
	...
	_, err := newConfigForTest(nil, 5, 8, 6)
	require.Error(t, err)
	require.IsType(t, InvalidConfigError{}, err)

	c, err := newConfigForTest(nil, 2, 4, 32)
	require.NoError(t, err)
...
func TestPositionIsOnPathToKey(t *testing.T) {
	...
	tests := []struct {
		...
		{config1bit, "10", []byte{0b0000_0000}, true},
		{config1bit, "10", []byte{0b1000_0000}, false},
		{config1bit, "10", []byte{0b0000_0001}, true},
	}
...
func TestPathComputation(t *testing.T) {
	cfg, _, _ := getTreeCfgsWith1_2_3BitsPerIndexUnblinded(t)

	tt := []struct {
		k   []byte
		exp []int
	}{
		{[]byte{0b0000_0000}, []int{0, 0, 0, 0, 0, 0, 0, 0}},
		{[]byte{0b1000_0000}, []int{0, 0, 0, 0, 0, 0, 0, 1}},
		{[]byte{0b1010_0000}, []int{0, 0, 0, 0, 0, 1, 0, 1}},
	}

	for _, test := range tt {
		t.Run(fmt.Sprintf("test"), func(t *testing.T) {
			p, err := cfg.getDeepestPositionForKey(Key(test.k))
			require.NoError(t, err)

			path := cfg.positionToChildIndexPath(p)

			var ci []ChildIndex
			for _, p := range test.exp {
				ci = append(ci, ChildIndex(p))
			}
			require.Equal(t, path, ci)
		})
	}
}
