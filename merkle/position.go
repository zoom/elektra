...
var _ sql.Scanner = &Position{}

func (p *Position) Scan(src interface{}) error {
	srcBytes, ok := src.([]byte)
	if !ok {
		return fmt.Errorf("need bytes to decode into position")
	}
	*p = *NewPositionFromBytes(srcBytes)
	return nil
}
...
// t.ChildrenPerNode - 1. It populates the slice with the siblings of p, and
...
func (t *Config) getDeepestPositionForKey(k Key) (*Position, error) {
	if len(k) != t.KeysByteLength {
		return nil, fmt.Errorf("Bad key len %d != %d", len(k), t.KeysByteLength) //NewInvalidKeyError()
	}
	var p Position
...
func (t *Config) positionToChildIndexPath(p *Position) (path []ChildIndex) {
	buf := make([]ChildIndex, t.getLevel(p))
	t.positionToChildIndexPathPlace(p, buf)
	return buf
}

// positionToChildIndexPath returns the list of childIndexes to navigate from the
// root to p (in reverse order).
func (t *Config) positionToChildIndexPathPlace(p *Position, buf []ChildIndex) (path []ChildIndex) {
	if t.ChildrenPerNode == 2 {
		for i := range buf {
			buf[i] = ChildIndex(((*big.Int)(p)).Bit(i))
		}
		return buf
	}
	...
	for i := range buf {
		buf[i] = ChildIndex(((*big.Int)(buff)).Bits()[0] & bitMask)
